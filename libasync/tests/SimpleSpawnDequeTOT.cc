...
class SimpleSpawnDequeTOT : public ::testing::Test {
protected:
	SimpleSpawnDequeTOT() {}
	virtual ~SimpleSpawnDequeTOT() {}
	virtual void SetUp() {
		q=new SimpleSpawnDeque<A>();
		count=0;
	}
	virtual void TearDown() {
		delete q;
	}
	SimpleSpawnDeque<A>* 		q;
	std::atomic<int> 		count;  // how many are stolen
};

TEST_F(SimpleSpawnDequeTOT, SingleThreadSituationA) {
...
TEST_F(SimpleSpawnDequeTOT, SingleThreadSituationInt) {
	auto qq=new SimpleSpawnDeque<int>();
...
TEST_F(SimpleSpawnDequeTOT, StealerRace) {
	for(int i=1;i<=100;i++){
